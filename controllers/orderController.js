...
const addOrder = async (req, res) => {
  ...
  if (checkItem) {
    ...
    try {
      if((Number(checkItem.quantity) + Number(quantity)) > Number(price.productQuantity)){
        return res.status(400).json({error:"Not enough stock to process this order. Only "+(Number(price.productQuantity) - Number(checkItem.quantity))+" pieces remaining."});
      }
  ...
  try {
    ...
    if(checkQuantity.productQuantity < quantity){
      return res.status(400).json({error:"Not enough stock to process this order. Only "+checkQuantity.productQuantity+" pieces remaining."});
    }
